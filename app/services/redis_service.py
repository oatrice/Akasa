...
def _normalize_project_path(project_path: str) -> str:
    """Normalize and validate a bound project path."""
...
def _get_project_path_key(chat_id: int, project_name: str) -> str:
...
async def set_project_path(chat_id: int, project_name: str, project_path: str) -> str:
    """Bind a project name to an absolute path."""
    normalized_project = _normalize_project_name(project_name)
    normalized_path = _normalize_project_path(project_path)
    key = _get_project_path_key(chat_id, normalized_project)

    await redis_pool.set(key, normalized_path, ex=settings.REDIS_TTL_SECONDS)
