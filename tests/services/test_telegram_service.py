...
async def test_send_proactive_message_success(mock_redis):
    ...
    with patch.object(tg_service, 'send_message', new_callable=AsyncMock) as mock_send_message:
...
async def test_send_proactive_message_bot_blocked(mock_redis):
    ...
    with patch.object(tg_service, 'send_message', new_callable=AsyncMock, side_effect=side_effect) as mock_send_message:
